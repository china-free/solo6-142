...
import { escapeText, fold, isValidMimeType, resolveType, } from './utils/functions.js';
import * as constants from './utils/constants.js';
export default class VCard {
    /**
     * Default Charset.
     */
    charset = constants.DEFAULT_CHARACTER_SET;
    /**
     * Default ContentType.
     */
     ...
     */
    fileExtension = constants.DEFAULT_EXTENSION;
    /**
     * Properties.
     */
    properties = [];
     ...
     */
    multiplePropertiesForElementAllowed = constants.ALLOWED_MULTIPLE_PROPERTIES;
    /**
     * Add address.
     *
     * @link   https://tools.ietf.org/html/rfc2426#section-3.2.1
     */
    addAddress({ postOfficeBox = '', extended = '', street = '', locality = '', region = '', postalCode = '', country = '', type = ['intl', 'postal', 'parcel', 'work'], } = {}) {
        const value = [
            postOfficeBox,
            extended,
            street,
     ...
     */
    addMediaUrl(property, url, element) {
        this.setProperty({
            element,
     ...
     */
    addMediaContent(property, content, mime, element) {
     ...
     */
    addName({ familyName = '', givenName = '', additionalNames = '', honorificPrefix = '', honorificSuffix = '', } = {}) {
        const values = [
            honorificPrefix,
            givenName,
            additionalNames,
    ...
    addLogo(options) {
        if ('url' in options) {
            ...
            this.addMediaContent('LOGO', options.image, options.mime ?? constants.DEFAULT_MIME_TYPE, 'logo');
        }
        return this;
    }
    /**
    ...
    addPhoto(options) {
        if ('url' in options) {
            ...
            this.addMediaContent('PHOTO', options.image, options.mime ?? constants.DEFAULT_MIME_TYPE, 'photo');
        }
        return this;
    }
    /**
     ...
     * @throws VCardException
     */
    addGeo({ latitude, longitude }) {
        if (latitude < -90 || latitude > 90) {
            throw new VCardException(`Invalid latitude: ${latitude}. Must be between -90 and 90.`);
        }
        if (longitude < -180 || longitude > 180) {
            throw new VCardException(`Invalid longitude: ${longitude}. Must be between -180 and 180.`);
        }
        this.setProperty({
            element: 'geo',
            key: 'GEO',
     ...
     */
    addLabel({ label, type = ['intl', 'postal', 'parcel', 'work'], }) {
        const resolved = resolveType(type);
        this.setProperty({
            element: 'label',
     ...
     */
    addCustomProperty({ name, value, params = '', group, }) {
        this.setProperty({
            element: 'custom',
    ...
    buildVCard() {
        ...
        for (const property of this.properties) {
            ...
            const charset = constants.TEXT_ELEMENTS.has(property.element)
                ? charsetStr
                : '';
            parts.push(fold(`${prefix}${property.key}${charset}:${property.value}\r\n`));
        }
     ...
     * @throws VCardException
     */
    setProperty({ element, key, value, group, }) {
        if (!this.multiplePropertiesForElementAllowed.has(element) &&
